...
# ---------------------------------------------------------------------------
# Production model: train on ALL studies for one EFO/condition, save with
# the exact X_cols it expects, so it can be used to score brand-new samples
# with no held-out test split eating into training data.
# ---------------------------------------------------------------------------

def select_best_fold_params(params_list: list, scores: list) -> dict:
    """Pick the hyperparameter set from whichever fold scored highest,
    rather than arbitrarily taking fold 0."""
    best_idx = int(np.argmax(scores))
    return params_list[best_idx]


def train_production_model(condition: str, dfs: dict, model_factory, params: dict,
                            X_cols: list):
    """Fit one final model for `condition` on every available sample for
    that condition -- no held-out study, no validation split. This is the
    model you ship for scoring new/external samples, not for reporting
    generalization performance (use evaluate_model's held-out AUCs for that)."""
    df = dfs[condition]
...
def save_production_model(output_dir: str, condition: str, clf, X_cols: list,
                           model_type: str, params: dict, n_samples: int) -> str:
    payload = {
        'model':      clf,
        'model_type': model_type,
        'params':     params,
        'X_cols':     X_cols,        # exact feature columns/order this model expects
        'condition':  condition,
        'n_samples':  n_samples,
    }
    safe_name = condition.replace('/', '_').replace(' ', '_')
    path = os.path.join(output_dir, f'production_model_{safe_name}.pkl')
    with open(path, 'wb') as f:
        pkl.dump(payload, f)
    print(f"[production] {condition}: saved -> {path}  "
          f"(model_type={model_type}, n_samples={n_samples})")
    return path


def train_and_save_production_models(
    dfs: dict,
    X_cols: list,
    best_params_by_type: dict,   # {'xgb': xgb_best_params, 'lr': lr_best_params, ...}
    metrics_by_type: dict,       # {'xgb': xgb_metrics, 'lr': lr_metrics, ...}
    model_factories: dict,       # {'xgb': make_xgb, 'lr': make_lr, ...}
    output_dir: str,
    conditions: list = None,
) -> dict:
    """
    For each requested condition (EFO):
      1. Pick whichever model TYPE (xgb/lr/rf) had the best mean held-out
         AUC for that condition (from the LOSO evaluation already run).
      2. Within that model type, pick the hyperparameters from whichever
         fold scored highest (not an arbitrary fold).
      3. Retrain on ALL samples for that condition (every study included,
         nothing held out) using those hyperparameters.
      4. Save {model, X_cols, model_type, params, condition, n_samples}.

    Returns {condition: saved_path}.
    """
    ...
    for condition in conditions:
        mean_aucs = {
            mtype: float(np.mean(metrics_by_type[mtype][condition]['scores']))
            for mtype in metrics_by_type
        }
        best_type = max(mean_aucs, key=mean_aucs.get)
        print(f"[production] {condition}: model-type AUCs = "
              f"{ {k: round(v, 4) for k, v in mean_aucs.items()} } "
              f"-> using {best_type}")

        scores = metrics_by_type[best_type][condition]['scores']
        params_list = best_params_by_type[best_type][condition]
        best_fold_params = select_best_fold_params(params_list, scores)

        clf = train_production_model(
            condition, dfs, model_factories[best_type], best_fold_params, X_cols
        )
        path = save_production_model(
            output_dir, condition, clf, X_cols, best_type, best_fold_params,
            n_samples=len(dfs[condition]),
        )
        production_paths[condition] = path
...
def main(cfg: DictConfig):
    ...
    # ====================================================================
    # Production models: train on ALL studies, save model + X_cols
    # ====================================================================
    # cfg.target_efo (optional): restrict to a single condition, e.g.
    # "condition_EFO0000305". If unset/absent, a production model is
    # trained and saved for every condition in the dataset.
    target_efo = cfg.get('target_efo', None) if hasattr(cfg, 'get') else None
    target_conditions = [target_efo] if target_efo else conditions

    print("\n" + "=" * 60)
    print(f"  Training production model(s) on ALL studies: {target_conditions}")
    print("=" * 60)

    production_paths = train_and_save_production_models(
        dfs=dfs,
        X_cols=X_cols,
        best_params_by_type={'xgb': xgb_best_params, 'lr': lr_best_params, 'rf': rf_best_params},
        metrics_by_type={'xgb': xgb_metrics, 'lr': lr_metrics, 'rf': rf_metrics},
        model_factories={'xgb': make_xgb, 'lr': make_lr, 'rf': make_rf},
        output_dir='../outputs',
        conditions=target_conditions,
    )
    with open('../outputs/production_model_paths.pkl', 'wb') as f:
        pkl.dump(production_paths, f)
